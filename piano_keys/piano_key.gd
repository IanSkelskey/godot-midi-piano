...
func setup(pitch_index: int):
	name = "PianoKey" + str(pitch_index) # Set the node name to uniquely identify the key.
	var exponent := (pitch_index - 69.0) / 12.0 # Calculate the exponent for the pitch scale calculation.
	pitch_scale = pow(2, exponent) # Calculate the pitch scale using the exponent.
...
func activate(velocity: int):
	...
	audio.volume_db = 20 * log(velocity / 127.0) # Adjust the volume based on the MIDI velocity.
